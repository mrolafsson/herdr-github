...
t=$(mktemp -d "${TMPDIR:-/tmp}/herdr-github-test.XXXXXX")
trap 'rm -rf "$t"' EXIT
...
[ "$("$t/repo/bin/herdr-github")" = "stand-in binary" ] || { echo "FAIL: good release not installed"; exit 1; }
[ "$(ls "$t/repo/bin")" = "herdr-github" ] || { echo "FAIL: extracted more than the binary"; exit 1; }
...
[ ! -e "$t/repo/bin/herdr-github" ] || { echo "FAIL: tampered binary left behind"; exit 1; }
