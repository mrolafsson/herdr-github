...
// the picker's background then, so the selected row stays visible. GitHub's
// own colours (labels) stay GitHub's.
func useTheme(p *palette) {
	...
	styleSelected, styleWarn, styleMerged = defaultStyleSelected, defaultStyleWarn, defaultStyleMerged
	if p == nil {
		return
	}
	...
	styleWarn = fg(styleWarn, p.Yellow)
	styleMerged = fg(styleMerged, p.Mauve)
	if p.SelectionBG != "" {
